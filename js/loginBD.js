    ...
    if (loginForm) {
        loginForm.addEventListener("submit", (e) => {
            ...
            if (validUser) {
                ...
                } else if (identificadorRol === 's') {
                    // --- NUEVA FUNCIÓN: SECRETARÍA ---
                    rolAsignado = "Secretaria";
                    destinoURL = "HomeAdministrativos.html"; // Asegúrate de que este archivo exista
                } else {
                    Swal.fire({
                        title: 'Error de Rol',
                        text: 'No se detectó un rol válido (a, d, e, s) antes del @.',
                        icon: 'error'
                    });
                    return;
                }
                ...
                // El Administrador y Secretaría siempre pueden entrar. Docentes y Estudiantes requieren activo === true
                if (rolAsignado !== "Administrador" && rolAsignado !== "Secretaria" && validUser.activo !== true) {
                    Swal.fire({
